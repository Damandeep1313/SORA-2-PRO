...
//   // CORRECTED: Remove the bolding asterisks (**)
...
//     // 2. Multi-Image-to-Video (I2V) Logic (Passing array of URLs)
...
//     // Ensure the output URL is explicitly converted to a string.
...
//     res.json({
//       status: 'completed',
//       cloudinary_video_url: cloudinaryResult.secure_url, 
//       model: MODEL_ID,
//     });
...
const validateReplicateKey = (req, res, next) => {
  ...
  if (!apiKey) {
    return res.status(401).json({
      error: "Unauthorized",
      message:
        "Missing X-REPLICATE-API-KEY header. Please provide your Replicate API token.",
    });
  }
...
// --- CORE ENDPOINT: GENERATE, UPLOAD, AND RETURN CLOUDINARY LINK ---
app.post("/generate-video", validateReplicateKey, async (req, res) => {
  const { prompt, duration_seconds, aspect_ratio, reference_image_url } =
    req.body;
  ...
  try {
    ...
    console.log(
      `[REQUEST] Submitting ${MODEL_ID} job. Waiting for video generation...`
    ); // 2. Call Replicate API

    const replicateOutput = await replicate.run(MODEL_ID, { input }); // 🔥 FIX FOR TypeError: Ensure the output URL is explicitly converted to a string. // Cloudinary's underlying library requires a string, not a Node.js URL object.
    const videoUrlFromReplicate = String(replicateOutput.url());
    console.log(
      `[SUCCESS] Replicate job complete. Video URL: ${videoUrlFromReplicate}`
    ); // 3. Upload Video from Remote URL to Cloudinary

    console.log("[UPLOAD] Uploading video to Cloudinary from remote URL..."); // This leverages Cloudinary's ability to pull from a remote URL, resolving // MIME-type issues as Cloudinary handles the fetch directly.
    const cloudinaryResult = await cloudinary.uploader.upload(
      videoUrlFromReplicate,
      {
    ...
    console.log(
      `[SUCCESS] Cloudinary upload complete. Public URL: ${cloudinaryResult.secure_url}`
    );
    ...
    console.error(`[ERROR] Generation or Upload failed:`, error); // Log the configuration error if Cloudinary is misconfigured (shows "your_cloud_name")
    if (process.env.CLOUDINARY_CLOUD_NAME === "your_cloud_name") {
      console.error(
        "!!! CLOUDINARY CONFIGURATION ERROR: Check your .env file !!!"
      );
    }
